  ...
  const ai: ResearchCapabilityProvider[] = [
    {
      ...
      detail: status.ai.configured
        ? `AI 摘要/证据抽取模型已配置：${status.ai.model}`
        : "缺少可用 OPENAI_API_KEY 或 GEMINI_API_KEY，报告会走 fallback。",
    },
  ];
...
function buildEnvChecklist(config: ResearchConfig, status: ReturnType<typeof getResearchConfigStatus>): ResearchEnvRequirement[] {
  return [
    ...
    envRequirement({
      ...
      group: "ai",
      requiredLevel: "at-least-one",
      configured: status.ai.openaiConfigured,
      requiredFor100: true,
      impact: "文档相关性判断、claim/evidence 抽取、冲突判断和中文报告生成。",
      howToGet: "在 OpenAI Platform 的 API keys 页面创建项目 key。",
    }),
    envRequirement({
      name: "GEMINI_API_KEY",
      group: "ai",
      requiredLevel: "at-least-one",
      configured: status.ai.geminiConfigured,
      requiredFor100: true,
      impact: "可替代 OpenAI 的 AI 摘要、证据抽取和报告生成。",
      howToGet: "在 Google AI Studio 或 Google Cloud 创建 Gemini API key。",
    }),
    envRequirement({
      ...
      requiredFor100: false,
      impact: "比赛数据集和 Kaggle 公开数据源检索/下载的用户名。",
      howToGet: "Kaggle Account 页面创建 API token 后使用 token JSON 里的 username。",
    }),
    envRequirement({
      name: "KAGGLE_KEY",
      group: "data",
      ...
      requiredFor100: false,
      impact: "比赛数据集和 Kaggle 公开数据源检索/下载的 token。",
      howToGet: "Kaggle Account 页面 Create New API Token，使用 token JSON 里的 key。",
    }),
    envRequirement({
      name: "FRED_API_KEY",
      group: "data",
